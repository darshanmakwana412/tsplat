...
const ITEMS: &[Entry] = &[
    Entry {
        group: Some("Splats"),
        item: HudItem::MaxSplats,
        label: "max_splats",
    },
    Entry {
        group: None,
        item: HudItem::Sigmoid,
        label: "sigmoid",
    },
    Entry {
        group: Some("Display"),
        item: HudItem::RenderBackend,
        label: "backend",
    },
    Entry {
        group: None,
        item: HudItem::PixelDensity,
        label: "px density",
    },
    Entry {
        group: Some("Perf"),
        item: HudItem::NumThreads,
        label: "threads",
    },
    Entry {
        group: Some("Camera"),
    ...
    Entry {
        group: None,
        item: HudItem::TranslateSpeed,
        label: "move spd",
    },
    Entry {
        group: None,
        item: HudItem::RotationSpeed,
        label: "rot spd",
    },
    Entry {
        group: None,
        item: HudItem::ZoomStep,
        label: "zoom spd",
    },
    Entry {
        group: Some("Render"),
    ...
    Entry {
        group: None,
        item: HudItem::AlphaThreshold,
        label: "alpha thr",
    },
    Entry {
        group: None,
        item: HudItem::ExtendSigma,
        label: "sigma ext",
    },
    Entry {
        group: None,
        item: HudItem::Saturation,
        label: "saturation",
    },
];
...
impl HudState {
    ...
    fn adjust(&mut self, dir: i8) -> HudAction {
        ...
        match item {
            ...
            HudItem::RotationSpeed => {
                self.rotation_speed = (self.rotation_speed + dir as f32 * 0.01).clamp(0.01, 0.50);
                HudAction::ValueChanged
            }
            HudItem::ZoomStep => {
    ...
    pub fn render(&self, camera: &OrbitCamera, di: &DisplayInfo, out: &mut String) {
        ...
        let info_rows: &[(&str, String)] = &[
            ...
            (
                "cell size",
                format!("{} x {} px", di.cell_size.w, di.cell_size.h),
            ),
